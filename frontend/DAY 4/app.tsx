...
function GroceryAssistantUI({ session }: { session: any }) {
  ...
  // ==========================================================
  // IMPORTANT:
  // Remember which USER message was already processed.
  //
  // This prevents:
  //
  // User says: "5 kg rice"
  // AI replies: "Sure, I have added rice."
  //
  // from causing the basket parser to process the same request
  // again.
  //
  // It also prevents a manually deleted item from being
  // automatically re-added when the AI replies.
  // ==========================================================

  const lastProcessedUserMessageRef =
  ...
  const messages = useMemo(() => {
    ...
    transcripts.forEach((t) => {
      ...
      const key =
        `stt:${isAgent ? 'agent' : 'user'}:${text.toLowerCase()}`;
    ...
    chatMessages.forEach((msg) => {
      ...
      const key =
        `chat:${isAgent ? 'agent' : 'user'}:${text.toLowerCase()}`;
    ...
    rawEntries.forEach((entry) => {
      ...
      if (
        !messageTimestampsRef.current.has(entry.key)
      ) {
      ...
      list.push({
        ...
        sender: entry.sender,
        timestamp:
          messageTimestampsRef.current.get(entry.key)!,
      });
    });
  ...
  //
  // IMPORTANT:
  // ONLY USER MESSAGES can modify the basket.
  //
  // ADD:
  // "5 kg rice"
  // "I need milk"
  ...
  useEffect(() => {
    // Find the latest USER message only.
    ...
    // --------------------------------------------------------
    // Prevent processing the same user message twice.
    // --------------------------------------------------------

    if (
      lastProcessedUserMessageRef.current ===
      latestUserMsg.id
    ) {
      return;
    }
    ...
    const hasRice =
      ...
      text.includes('rice') ||
      text.includes('arisi') ||
      text.includes('பச்சை அரிசி');

    const hasBajji =
      text.includes('பஜ்ஜி') ||
      text.includes('bajji') ||
      text.includes('bajji mix');

    const hasTea =
      text.includes('டீ') ||
      text.includes('tea') ||
      text.includes('chai') ||
      text.includes('தேயிலை');

    const hasBiscuit =
      text.includes('பிஸ்கட்') ||
    ...
    const removeIntent =
      // English
      /\b(remove|delete|discard|cancel|take\s*off|take\s*out)\b/i.test(
        text
      ) ||

      // English "don't need"
      /\b(don't|do not)\s+(need|want)\b/i.test(
        text
      ) ||

      // Tanglish
      /\b(vendaam|vendam|venam|venamnu)\b/i.test(
        text
      ) ||

      /\b(neekku|neekunga|neekavum|eduthudu|eduthunga)\b/i.test(
        text
      ) ||

      /\b(remove|delete)\s*(pannu|pannunga|pannidu|pannunga)\b/i.test(
        text
      ) ||

      // Tamil
      text.includes('வேண்டாம்') ||
      text.includes('வேணாம்') ||
      text.includes('நீக்கு') ||
      text.includes('நீக்குங்க') ||
      text.includes('நீக்கவும்') ||
      text.includes('எடுக்கவும்') ||
      text.includes('எடுத்துவிடு') ||
      text.includes('எடுத்துடுங்க');
    ...
    //
    // IMPORTANT:
    // Return immediately after removal.
    //
    // This prevents:
    //
    // "remove rice"
    //
    // from reaching the ADD logic below.
    // ========================================================
    ...
    const parseQuantityText = (
      ...
      const digitMatch = rawText.match(
        /(\d+(?:\.\d+)?)\s*(kg|g|l|ml|packet|pack|packets)?/i
      );
      ...
      if (digitMatch) {
        ...
        if (detectedUnit === 'kg') {
          ...
          detectedUnit === 'packet' ||
          detectedUnit === 'pack' ||
          detectedUnit === 'packets'
        ) {
          unit = 'PACKET';
        }
      } else {
        // ----------------------------------------------------
        // Tamil / English number words
        // ----------------------------------------------------
      ...
      if (
        /\bkg\b/i.test(rawText) ||
        rawText.includes('கிலோ')
      ) {
        unit = 'KG';
      } else if (
        /\bg\b/i.test(rawText) ||
        rawText.includes('கிராம்')
      ) {
        unit = 'G';
      } else if (
        /\bl\b/i.test(rawText) ||
        /\blitre\b/i.test(rawText) ||
        /\bliter\b/i.test(rawText) ||
        rawText.includes('லிட்டர்')
      ) {
        unit = 'L';
      } else if (
        /\bml\b/i.test(rawText) ||
        rawText.includes('மில்லி')
      ) {
        unit = 'ML';
      } else if (
        /\bpacket\b/i.test(rawText) ||
        /\bpack\b/i.test(rawText) ||
        rawText.includes('பாக்கெட்')
      ) {
        unit = 'PACKET';
      }
    ...
    const updateOrAddItem = (
      ...
      // ------------------------------------------------------
      // Detect explicit quantity
      // ------------------------------------------------------

      const hasNumber =
        /\d/.test(text) ||
        /(one|two|three|four|five|six|seven|eight|nine|ten)/i.test(
          text
        ) ||
        /(ஒரு|ஒன்று|ரெண்டு|இரண்டு|மூணு|மூன்று|நாலு|நான்கு|அஞ்சு|ஐந்து|ஆறு|ஏழு|எட்டு|ஒன்பது|பத்து)/.test(
          text
        );
  ...
  const handleStart = async () => {
    ...
    // Allow the next call to process its first message.
